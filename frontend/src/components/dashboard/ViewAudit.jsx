...
const Users = ({
  ...
  >
    ...
    <div className="w-1/6 flex space-x-6">
      ...
      <button
        onClick={onDelete}
        className="text-palette-red hover:text-red-900 cursor-pointer"
      >
        <Trash2 size={18} />
      </button>
    </div>
...
export default function ViewAudit() {
        ...
        {["OR", "AND"].map((mode) => (
          <button
            ...
            className={`px-2 py-1 rounded-md text-xs font-semibold ${
              matchMode === mode ? "bg-blue-600 text-white" : "bg-zinc-200"
            }`}
          >
            {mode}
          </button>
  ...
  const filteredAudits = audits.filter((audit) => {
    ...
    const name = `${audit.applicant?.first_name || ""} ${
      audit.applicant?.surname || ""
    }`;
    ...
    const readablePriorities = Object.entries(audit.special_circumstances || {})
      .filter(([_, value]) => value === true)
      .map(
        ([key]) => SPECIAL_CIRCUMSTANCE_LABELS?.[key] || key.replace(/_/g, " ")
      );
  ...
  const deleteAudit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.REACT_APP_BASE_URL}/api/audits/${auditToDelete._id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-2">
            ...
            <div className="w-full md:w-2/5">
              <Select
                ...
                options={priorityOptions.map((label) => ({
                  value: label,
                  label,
                }))}
                value={searchPriority.map((label) => ({ value: label, label }))}
                onChange={(selected) =>
                  setSearchPriority(selected.map((s) => s.value))
                }
                ...
                filteredAudits.map((audit, index) => {
                  ...
                  const priority =
                    Object.entries(audit.special_circumstances || {})
                      ...
                      .join(", ") || "None";
                  const period =
                    calculateYearsBetweenDates(audit.application_date) || "N/A";
